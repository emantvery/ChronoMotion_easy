...
```powershell
# 批量预测全部文件
python predict.py --model best_multimodal_model.pth --input ../../csv数据 --output ./results --device cuda

# 只预测前 200 个文件（快速测试用）
python predict.py --model best_multimodal_model.pth --input ../../csv数据 --output ./results --device cuda --max_files 200
```

> `--max_files N` 限制批量预测只处理前 N 个文件，不加此参数则处理全部。结果保存在 `--output` 指定的文件夹下的 `predictions.json`。
...
浏览器打开 `http://localhost:5000`，在画布上点击添加观测点（至少 20 个），点击"开始预测"查看 3 条候选轨迹。页面支持中文 / English 切换，预测完成后自动显示轨迹详情（概率、终点坐标、总距离）和置信度等级。
...
**Q: 批量预测 predictions.json 是空的？**
A: CSV 文件中 AGENT 的数据点不足 20 个就会被跳过。确认输入文件夹中的 CSV 包含足够的轨迹数据（至少 20 个 AGENT 时间点）。可以用 `--max_files 10` 先测试少量文件排查问题。
